...
| [`internal-linking`](internal-linking/SKILL.md) | Audits and optimizes one article's internal links: builds the link universe from the live sitemap, preserves/verifies every existing link, adds contextual links to topic-cluster siblings, enforces the 3-link floor + responsive caps, and runs the evidence-based 8-check link audit. Distilled from the Content Editor Agent's link rules. |
