...
| [`new-post-int-linking`](new-post-int-linking/blog-internal-linking-new-post-SKILL.md) | Plans internal links into a NEW post before drafting: builds the link universe from the live sitemap, maps blog-sibling + service-page links into the outline, weaves them in with natural keyword-rich anchors, and verifies everything with the shared 8-check audit. |
| [`blog-int-linking`](blog-int-linking/blog-internal-linking-audit-SKILL.md) | Audits and optimizes one article's internal links: builds the link universe from the live sitemap, preserves/verifies every existing link, adds contextual links with an explicit blog-sibling + service-page mix and natural, keyword-rich anchors, enforces the 3-link floor + responsive caps, and runs the evidence-based 8-check link audit. Distilled from the Content Editor Agent's link rules. |

## File naming

The two internal-linking skills use descriptive filenames (`blog-internal-linking-audit-SKILL.md`,
`blog-internal-linking-new-post-SKILL.md`) so they stay distinguishable when shared as standalone
files into someone else's skill collection. **To install one in Claude Code, the file must be named
`SKILL.md` inside a folder named after the skill** — either rename the copy, or symlink it:

```bash
mkdir -p ~/.claude/skills/blog-int-linking
ln -s "$(pwd)/blog-int-linking/blog-internal-linking-audit-SKILL.md" ~/.claude/skills/blog-int-linking/SKILL.md
```
